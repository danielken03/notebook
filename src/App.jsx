...
export default function App() {
  ...
  function create() {
    ...
    node.children[name] = creating === "folder"
  ...
  const s = {
    ...
    header: { display: "flex", alignItems: "center", gap: 8, marginBottom: 32, flexWrap: "wrap" },
    ...
    del: { background: "none", border: "none", cursor: "pointer", color: "#ddd", fontSize: 13, fontFamily: FONT, padding: "0 4px" },
    ...
    <div style={s.app}>
      ...
      {items.map(([name, node]) => (
        <div key={name} style={s.row}>
          ...
          <button style={s.name} onClick={() => node.type === "folder" ? setPath([...path, name]) : setOpenPage({ name, content: node.content })}>{name}</button>
          <button style={s.del} onClick={() => deleteItem(name)}>✕</button>
        </div>
      ))}
